...
struct SessionStageProcessStack: View {
        ...
        Button(action: action) {
            ZStack(alignment: .topLeading) {
                ForEach(Array(visibleProcesses.enumerated()), id: \.element.id) { index, process in
                    processLayer(process, index: index)
                }
            }
            .frame(width: 164, height: max(118, CGFloat(visibleProcesses.count) * 42 + 72))
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
        .onHover { isHovering = $0 }
        .accessibilityLabel("Related Windows processes")
        .accessibilityValue("\(processes.count)")
    }

    private var visibleProcesses: [WindowsProcessSnapshot] {
        Array(processes.filter { !$0.name.isEmpty }.prefix(7))
    }

    private func processLayer(_ process: WindowsProcessSnapshot, index: Int) -> some View {
        HStack {
            Text(process.name)
                .font(.system(size: 10, weight: index == 0 ? .semibold : .regular))
                .foregroundStyle(.white.opacity(index == 0 ? 0.92 : 0.72))
                .lineLimit(1)
            Spacer(minLength: 0)
        }
        .padding(.horizontal, 12)
        ...
        .overlay {
            ...
            .strokeBorder(
                index == 0 && (isSelected || isHovering)
                    ? Color.blue.opacity(0.68)
                    : Color.white.opacity(0.14)
            )
        }
        .offset(x: CGFloat(index) * 3, y: CGFloat(index) * 41)
        .zIndex(Double(visibleProcesses.count - index))
    }
}
