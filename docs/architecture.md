...
`runtime/runner` is the only boundary that executes Wine and Windows workloads. It accepts a serialized `CommandPlan`, constructs an explicit process environment, streams output, handles cancellation, and returns the child status. The runner retains structured output in a protected, bounded per-container live journal. While the Logs screen is closed, the runner writes bounded batches without classifying repetitions and `AppStore` pauses journal decoding; opening Logs resumes off-main-thread tailing and compaction. Application-specific compatibility behavior belongs in the runtime source, not in the runner.
